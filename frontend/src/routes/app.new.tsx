...
function NewEntryPage() {
  ...
  const initialEntryDate = useRef(todayISO());
  const [entryDate, setEntryDate] = useState(initialEntryDate.current);
  ...
  const isDirty =
    ...
    entryDate !== initialEntryDate.current ||
    isPrivate !== true ||
    blocks.some(
      (b) =>
        b.text.trim().length > 0 ||
        !!b.url ||
        (b.caption ? b.caption.trim().length > 0 : false),
    );
  ...
  const blocker = useBlocker({
    shouldBlockFn: () => isDirty && !isRoutingRef.current,
